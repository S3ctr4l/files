...
DRY_RUN=0
INVASIVE_MODE=0
TIMEOUT_CMD="timeout 300"  # 5 minute timeout for long operations
...
log() {
    ...
    echo -e "[$timestamp] $msg" | tee -a "$LOGDIR/run.log"
}
...
run() {
    ...
    if [[ "$*" =~ (flashrom|ectool|i2cdump|dd.*/dev/mem) ]]; then
        eval "$TIMEOUT_CMD $@" 2>&1 | tee -a "$LOGDIR/${FUNCNAME[1]:-default}.log" || {
            warn "Command timed out or failed: $*"
            return 1
        }
    else
        eval "$@" 2>&1 | tee -a "$LOGDIR/${FUNCNAME[1]:-default}.log" || {
            warn "Command failed: $*"
            return 1
        }
    fi
...
phase_acpi_nuclear() {
    ...
    if check_tool "iasl"; then
        ...
        for aml in "$WORKDIR"/acpi/*.bin; do
            [[ -f "$aml" ]] || continue
            local basename=$(basename "$aml" .bin)
        ...
        for aml in "$WORKDIR"/acpi/*.dat; do
            [[ -f "$aml" ]] || continue
            local basename=$(basename "$aml" .dat)
            log "Decompiling $basename..."
            run iasl -d "$aml" 2>&1 | tee "$WORKDIR/acpi/${basename}_iasl.log" || warn "Failed to decompile $aml"
        done
...
# ---------------- ENSURE ALL PHASE FUNCTIONS EXIST ----------------
# Note: Many phase functions are defined in the original script but omitted here for brevity.
# In practice, you should include all original phase functions, applying similar fixes.
