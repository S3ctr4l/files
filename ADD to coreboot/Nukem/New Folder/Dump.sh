...
phase_acpi_nuclear() {
    ...
    check_tool acpidump && {
        run sudo acpidump -bfor check_tool iasl; then
        # List globs separately to be safe across all shells
        for aml in "$WORKDIR/acpi"/*.bin "$WORKDIR/acpi"/*.dat; do
            # Skip if no files match (prevents 'file not found' errors
            [ -e "$aml" ] || continue
            
            # Run the tool and hide output here, not on the 'for' line
            iasl -d "$aml" >/dev/null 2>&1
        done
    fi 
 
aml in "$WORKDIR/acpi"/*.bin "$WORKDIR/acpi"/*.dat; do
    # This check prevents errors if no .bin or .dat files exist
    [ -e "$aml" ] || continue
    
    iasl -d "$aml"
done -z -o "$WORKDIR/acpi/acpidump_all.dat"
        run sudo acpidump -s -o "$WORKDIR/acpi/acpidump_summary.txt"
    }

    # Decompile ALL tables
    if check_tool iasl; then
        for aml in "$WORKDIR/acpi"/*.{bin,dat} 2>/dev/null; do
            [ -f "$aml" ] || continue
            basename=$(basename "$aml" | sed 's/\.[^.]*$//')
            run iasl -d "$aml" -p "$WORKDIR/acpi/${basename}" 2>&1 | tee "$WORKDIR/acpi/${basename}_iasl.log"
        done
...
phase_logs() {
    ...
    # Summary report
    {
        ...
        [ -f "$WORKDIR/acpi/acpidump_all.dat" ] && echo "✓ ACPI
        log "${GREEN}=======================================${NC}"
...
This script now extracts **literally everything accessible** from your platform:

1. **Complete flash image** with verification reads
2. **All MSRs** from all CPU cores (0x0-0x2000 range)
3. **Every PCI device config space** (both standard and extended)
4. **Complete GPIO pad configuration**
5. **All ACPI tables** with full decompilation
6. **EC RAM** byte-by-byte
7. **Complete UEFI variable set**
8. **Secure Boot key chain**
9. **TPM measurements** and event log
10. **Every firmware blob** loaded by kernel
11. **All storage controller firmware versions**
12. **Network card EEPROMs**
13. **USB device descriptors**
14. **Audio codec registers**
15. **Sensor readings**
16. **Power management state**
17. **Bootloader configs**
18. **NVRAM/CMOS raw dump**
19. **SMM analysis** (with invasive mode)
20. **Physical memory regions** (with invasive mode)

## Usage
```bash
# Safe mode (skips invasive operations)
sudo ./nuclear_dump.sh

# Full nuclear mode (enables SMM/memory dumps)
sudo ./nuclear_dump.sh --invasive

# Test run
sudo ./nuclear_dump.sh --dry-run
```

## Safety Notes

**INVASIVE_MODE=1 operations can:**
- Hang the system (SMM access, /dev/mem reads)
- Trigger watchdog timers (SuperIO probing)
- Alter GPIO states (if reading causes side effects)
- Expose sensitive data (memory dumps)

**Use with caution on production systems.**

## What's Still Missing?

Things that **require external hardware** or **are impossible without physical access**:

1. **EC firmware flash read** (needs EC-specific protocol, risk of brick)
2. **ME firmware via JTAG** (requires Intel DCI or hardware debugger)
3. **CPU microcode from on-die ROM** (not extractable)
4. **Fuse values** (eFUSE/OTP regions, mostly locked)
5. **DRAM training parameters** (some in MRC cache, rest in memory controller)
6. **PCH strap configuration** (burned into PCH, only readable via descriptor)

---

This is **maximum depth firmware forensics**. You now have a coreboot replication baseline. Let me know if you need help parsing any specific artifacts or building the devicetree!======${NC}"
